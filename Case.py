...
def on_right_click(self, clicked:bool) :
    if (not clicked) :        
        self.set_flag((self.get_flag() + 1)%3)
        if (self.get_flag() == 0) :
            self.set_sprite(pygame.image.load("sprites/Grid.png").convert())
        if (self.get_flag() == 1) :
            self.set_sprite(pygame.image.load("sprites/flag.png").convert())
        if (self.get_flag() == 2) :
            self.set_sprite(pygame.image.load("sprites/question.png").convert())
...
class Case(pygame.sprite.Sprite) :
    def __init__(self, bomb:bool, grid_pos:tuple, matrix:Matrix.Matrix, group:pygame.sprite.Group, clicked=False, flag=0, num_bombs=-1):
        ...
        self.__sprite = ClickableSprite.ClickableSprite(not_clicked, grid_pos[0]*32, grid_pos[1]*32, lambda : on_left_click(self, lambda : matrix.click(pos=grid_pos), lambda : self.reset_sprites(matrix, group)), lambda : on_right_click(self, self.__clicked))
